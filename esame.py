...
class CSVTimeSeriesFile:
  ...
  def get_data(self):
    try:
      ...
      raise ExamException(f"File '{self.name}' non trovato.")
...
def detect_similar_monthly_variations(time_series, years):
  if len(years) != 2:
    raise ExamException(
        "Il valore di input 'years' deve contenere almeno 2 anni.")

  data_filtered = [
      item for item in time_series
      if years[0] <= int(item[0].split('-')[0]) <= years[1]
  ]
  ...
  # Riduci la lista di similar_variations a 11 valori, selezionando uno ogni due valori.
...
if __name__ == "__main__":
  ...
  try:
    ...
    for value in result:
      print(value)  # Stampa ciascun valore True o False su una nuova riga.
